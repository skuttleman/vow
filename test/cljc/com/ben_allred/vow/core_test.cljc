...
(deftest finally-test
  (testing "(finally)"
    (testing "when the promise succeeds"
      (let [cleanup (atom nil)
            result (-> 3
                       (vow/resolve)
                       (vow/finally #(reset! cleanup :clean))
                       (deref))]
    ...
    (testing "when the promise fails"
      (let [cleanup (atom nil)
            result (-> 3
                       (vow/reject)
                       (vow/finally #(reset! cleanup :clean))
    ...
    (testing "when the callback returns a rejected promise"
      (testing "rejects the promise"
        (is (= [:error 7] (-> 3
                              (vow/resolve)
                              (vow/finally #(vow/reject 7))
                              (deref))))))
